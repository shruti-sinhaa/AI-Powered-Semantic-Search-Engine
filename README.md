...
```
 3. Semantic Search Query The search is performed using the knn option to find the closest vectors in the latent space.
```
...
5) Index Data:
 ```
 Bash python index_data.py myntra_products_catalog.csv.csv
```
7) Search:
```
...
- Latency: Average search response time of $<100ms$ on a dataset of $10,000+$ records.
...
- Deployment: Wrap the search engine in a FastAPI or Flask REST API.
